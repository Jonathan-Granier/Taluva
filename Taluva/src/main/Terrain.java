...
public class Terrain {
	...
	// Renvoie vrai ssi le placement de cette tuile est autoris√© au point P.
	public boolean placement_tuile_autorise(Tuile tuile, Point P){
		//TODO on ne gËre pas le cas d'Ècraser une citÈ entiËre
		if(empty) return dans_terrain(tuile.getOrientation(),P);
		else{
			// Teste si la tuile est pos√©e sur d'autres tuiles
			int x = P.x;
			int y = P.y;
			int n0,n1,n2;
			...
			if(n0>0 || n1>0 || n2>0){
				...
				if(n0==n1 && n1==n2){
					// Si les 3 cases dessous sont au m√™me niveau
					// On joue alors sur des tuiles, on v√©rifie la disposition des volcans
					...
					// On essaye de jouer partiellement sur de tuiles ou de niveaux diff√©rents : interdit
					return false;
				}
			}
			else{
